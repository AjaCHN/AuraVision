...
- **Fluid Curves (极光之舞):**
  - 逻辑：叠加多层不同频率与相位的正弦波，模拟极光的流动感。每层拥有独立的速度乘数与垂直偏移。
  - 优化：基础波形频率降低约 30%（即波长拉长 50%），配合增强的视差速度差异，营造更宏大的视觉广度。
- **Macro Bubbles (微观液泡):**
...
- **Liquid Sphere (液态星球):**
  - 逻辑：采用多层分形噪声对 Icosahedron (二十面体) 顶点的位移，模拟液态金属的表面张力。场景采用 `RectAreaLight` (矩形区域光) 营造柔和高光。
...
---
*Aura Vision Rendering - Version 0.7.0*
